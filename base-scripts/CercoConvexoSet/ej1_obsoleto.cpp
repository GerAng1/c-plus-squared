...
void find_hull(const std::vector<gal::Point> &points, const gal::Point a,
               const gal::Point b, std::set<gal::Point> &convex_points, const int &side)
{
    int ind = -1;
    int max_dist = 0;
...
void print_hull(std::vector<gal::Point> &points)
{
    ...
    std::set<gal::Point> convex_points;
    ...
    {
        std::cout << *convex_points.begin() << std::endl;
...
int main(int argc, char const *argv[])
{
  // auto start = std::chrono::high_resolution_clock::now();
    ...
    p.set_xy(-2, -3); points.push_back(p);
